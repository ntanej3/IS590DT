...
###
# Use the tm package to transform the data to a Corpus and then to a DocumentTermMatrix.
# When transforming data to a DocumentTermMatrix, we apply the following steps: terms are stemmed 
# and the stop words, punctuation, numbers and terms of length less than 3 are removed (in the control argument).
...
# Goal: remove terms with low frequences as well as those occurring in many documents. 
# We only include terms whose tf-idf values are at least 0.1, which is a bit more than the median and 
...
term_tfidf = tapply(JSS_dtm$v/row_sums(JSS_dtm)[JSS_dtm$i], JSS_dtm$j, mean)*
  log10((1+nDocs(JSS_dtm))/(1+col_sums(JSS_dtm > 0)))
...
# 
# 1. a mixing weight vector (over 30 topics) for each document,
# 2. a term frequency vector (over all words) for each of the 30 topic,
# 3. and other summaries from this topic model.
...
wordcloud(d$word, d$freq, max.words=50, scale=c(3,.2), random.order=FALSE, rot.per=.3, colors=brewer.pal(8,"Dark2"))
??wordcloud
...
for(i in 1:6){
  ...
  myNames = names(v);
  d = data.frame(word=myNames, freq=v);
  wordcloud(d$word, d$freq, max.words=50, scale=c(4,.6), random.order=FALSE, rot.per=.25, 
            colors=brewer.pal(8,"Dark2"))
}
